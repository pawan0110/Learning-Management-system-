...
console.log("Average Rating:", avgRating);
  ...
  // If selectedCourseData isn't set from the list, fetch it directly from the server
  useEffect(() => {
  ...
  }, [courseId, selectedCourseData, dispatch]);
        ...
        <div className='w-full transition-all duration-300 py-5   flex items-start justify-center lg:justify-start flex-wrap gap-6 lg:px-20 '>
          
            {
                selectedCreatorCourse?.map((item,index)=>(
                  <Card key={index} thumbnail={item.thumbnail?.url} title={item.title} id={item._id} price={item.price} category={item.category}/>
                ))
            }
        </div>
      </div>

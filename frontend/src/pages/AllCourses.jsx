    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className={`w-[260px] h-screen overflow-y-auto bg-black fixed  top-0 left-0 p-6 py-[130px] border-r border-gray-200 shadow-md transition-transform duration-300 z-5 
        ...
        <form className="space-y-4 text-sm  bg-gray-600 border-white text-[white] border  p-5 rounded-2xl" onSubmit={(e)=>e.preventDefault()}>
          <button className='px-2.5 py-2.5  bg-black text-white  rounded-[10px] text-[15px] font-light flex items-center justify-center gap-2 cursor-pointer' onClick={()=>navigate("/searchwithai")}>Search with AI <img src={ai} className='w-[30px] h-[30px] rounded-full' alt="" /></button>
      ...
      <main className="w-full transition-all duration-300 py-[130px] md:pl-[300px]  flex items-start justify-center md:justify-start flex-wrap gap-6 px-2.5">
        ...
        filterCourses?.map((item,index)=>(
          <Card key={index} thumbnail={item.thumbnail?.url} title={item.title} price={item.price} category={item.category} id={item._id} reviews={item.reviews} />

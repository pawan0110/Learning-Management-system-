...
function AddCourses() {
  ...
  const editCourseHandler = async () => {
   ...
   formData.append("thumbnail", backendImage || "");
    ...
    try {
      ...
      let updatedCourses = courseData ? [...courseData] : [];
      ...
      if (!updatedCourses.some((c) => c._id === courseId)) {
  ...
  const removeCourse = async () => {
    ...
    try {
      ...
      console.log(error);
      toast.error(error?.response?.data?.message || "Remove course failed");
    } finally {
      setLoading(false);
    }
  };
        ...
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="relative w-[300px] h-[170px]">
            <img
              ...
              alt=""
              className="w-full h-full border border-black rounded-[5px] cursor-pointer object-cover"
              onClick={() => thumb.current.click()}
            />
            <MdEdit className="w-5 h-5 absolute top-2 right-2" onClick={() => thumb.current.click()} />
          </div>
